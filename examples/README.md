...
- The alert body includes the current run's structured failure report plus any stderr warnings emitted outside that report, such as `ZXFER_ERROR_LOG` append or post-append lock-release failures.
- By default the mailed and logged `invocation` / `last_command` fields are emitted as `[redacted]`. Set `ZXFER_UNSAFE_FAILURE_REPORT_COMMANDS=1` only for deliberate local debugging when you explicitly want those command fields emitted verbatim.
