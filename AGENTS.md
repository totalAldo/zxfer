...
- **Validate continuously:** run `./tests/run_shunit_tests.sh` after meaningful shell changes. Use targeted suites for faster iteration and run `tests/run_coverage.sh` when expanding tests. Do not run `tests/run_integration_zxfer.sh` from Codex; update the integration harness as needed and note that manual execution by a human is still required.
