...
- [x] `src/zxfer_get_zfs_list.sh:45-113` – run with `-j 1` and `-j >1` to hit both code paths in `write_source_snapshot_list_to_file`, including the remote/`zstd` compression branch by running with `-O localhost -z`. (Local `-j 2` path covered by `parallel_jobs_listing_test` when GNU parallel is available; remote/compressed path still outstanding.)  
...
- [x] `src/zxfer_zfs_send_receive.sh:44-89` – run with `-D 'pv -s %%size%% > /dev/null'` so `calculate_size_estimate` and `handle_progress_bar_option` wrap the send stream without breaking dataset creation. (Covered by `progress_wrapper_test`, using a harmless pass-through wrapper that does not emit to stdout.)  
