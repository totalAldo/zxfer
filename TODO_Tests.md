...
- [ ] `src/zxfer_zfs_mode.sh:279-334` – add a non-recursive `-N` test to prove child datasets are not touched, and another with `-R` to ensure recursion still works when child datasets share parents.  
...
- [ ] `src/zxfer_zfs_mode.sh:332-341` – run with `-s` (once with `-R` and once with `-N`) so the non-migration snapshot branch in `newsnap` executes, proving the auto-created snapshot is replicated.  
